...
const Articles = () => {
  ...
  const [articleCover, setArticleCover] = useState<any>({});
  ...
  const createArticle = (e: React.MouseEvent<HTMLInputElement>) => {
    e.preventDefault();
    const localStorageDate = JSON.parse(localStorage.getItem("user")!);
  ...
  const saveAsDraft = (e: React.MouseEvent<HTMLInputElement>) => {
    e.preventDefault();
    const localStorageDate = JSON.parse(localStorage.getItem("user")!);
    let formData = new FormData();
    formData.append("title", formState.inputs.title.value);
